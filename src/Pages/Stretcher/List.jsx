...
function StretcherList() {
  ...
  // 저장 모달
  const [isolationSaveModalObj, setIsolationSaveModalObj] = useState({
    show: false,
    data: {},
  ...
  const validationModalData = async (nullList, saveData) => {
    if (nullList.length > 1 && nullList.length > 0) {
      ...
      nullList[0] === "dschgeSchdldDate" &&
        alert("격리해제 예정일이 공백입니다.");
    ...
    if (saveData.cellPhone.current.value.length < 11) {
      ...
      saveData["admissionDate"].current.value >=
      saveData["dschgeSchdldDate"].current.value
    ) {
  ...
  const create = async (saveData) => {
    let nullList = Object.keys(saveData)
      ...
      .filter((value) =>
        value === "sex"
          ? saveData[value] === ""
          : saveData[value].current.value === ""
      );
    const validation = await validationModalData(nullList, saveData);
    if (validation) {
      let confirmStatus = await confirm(
        `${saveData["patientNm"].current.value}를 생성하시겠습니까?`
      );
      if (confirmStatus) {
          ...
          if (data.code === "00") {
            ...
            setPaginationObj((prevState) => ({
              ...
              prevPaginationExists:
                data.result.admissionListResponseByQuarantineVO.paginationInfoVO
                  .prevPaginationExists,
            ...
            setIsolationTableData(
              data.result.admissionListResponseByQuarantineVO
  ...
  const update = async (saveData) => {
    ...
    if (validation) {
      let confirmStatus = await confirm(
        `${saveData["patientNm"].current.value}를 수정하시겠습니까?`
      );
      if (confirmStatus) {
  ...
  /*추론오류 모달*/
  const [inferenceModal, setInferenceModal] = useState({
    show: false,
    data: {},
  });
  /*추론 오류 팝업 닫기 */
  ...
  //추론 오류 모달
  const [isolationLogModalObj, setIsolationLogModalObj] = useState({
    show: false,
    data: {},
  });
  //추론 오류 모달
  const handledIsolationLogModal = (admissionId) => {
    isolationApi
      .logDetail(admissionId)
      .then(({ data }) =>
        setIsolationLogModalObj({ show: true, data: { ...data.result } })
      );
  };

  const discharge = useCallback(
    async (admissionId, dischargeDate, quantLocation, patientNm) => {
      if (dischargeDate === "") {
        alert("격리해제일이 공백입니다.");
      } else {
        const confirmState = await confirm(
          `${patientNm} 을 격리해제 처리 하시겠습니까?`
        );
        if (confirmState) {
          isolationApi
            .discharge(admissionId, dischargeDate, quantLocation)
            .then(({ data }) => {
              if (data.code === "00") {
                alert(data.message);
  ...
  const isolationTableColumn = [
    ...
    { Header: "산소포화도", accessor: "spResult", vital: true },
    {
      ...
      editEvent: handledIsolationExitModal,
    },
    {
      Header: "영상다운",
      accessor: "videoDown",
    ...
    },
    {
      Header: "추론",
      accessor: "aiExe",
      editElement: "aiExeButton",
      editElementType: "Isolation",
      editEvent: handledInferenceModal,
    },
    {
      Header: "수정",
      accessor: "update",
      editElement: "updateButton",
      editElementType: "Isolation",
      editEvent: handledIsolationSaveModal,
    },
  ];
  // 검색 Input Enter 이벤트
  ...
  return (
    <>
                        ...
                        <div className="ms-auto">
                          <div className="btn_wrap d-flex">
                            ...
                            <button
                              ...
                              onClick={() => handledIsolationSaveModal("", "C")}
                            >
                              등록
                            </button>
      ...
      <>
        {/* 신규 수정 모달*/}
        <IsolationSaveModal
          isolationSaveModalObj={isolationSaveModalObj}
          handledClose={handledCloseIsolationSaveModal}
        />
      </>
      <>
        {/* 퇴소 모달*/}
      ...
      <>
        {/* 추론오류 모달 */}
        <InferenceErrorModal
          inferenceModal={inferenceModal}
          handledClose={handledCloseInterfaceExitModal}
        />
      </>
    </>
  );

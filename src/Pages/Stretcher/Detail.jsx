...
function AdmissionDetail() {
  ...
  const recordSave = async () => {
    ...
    if (!record) {
      ...
      if (confirmStatus) {
        admissionDetailApi
          .recordUpdate(medicalSeq, record, todayInput.current.value)
          .then(({ data }) => {
            if (data.code === "00") {
              alert(data.message);
      ...
      if (confirmStatus) {
        admissionDetailApi
          .addRecord(record, todayInput.current.value)
          .then(({ data }) => {
            if (data.code === "00") {
              alert(data.message);
              setRecordList(data.result);
            ...
            <div className="body">
              <div className="tab-content" id="pills-tabContent">
                <div
                  ...
                  id="scrollbar"
                  role={"recordList"}
                  style={{ overflow: "auto", height: "54vh" }}
                >
                  {tabCode == 0 ? (
                    <div>
                      {interviews &&
            ...
            <div className="footer">
              <form>
                ...
                <div className="btn_wrap d-flex">
                  <span className="byte">
                    <strong>{noticeText.length}</strong> / 500
                  </span>
                  <input
                    type="date"
                    className="form-control w200 ms-auto"
                    ref={todayInput}
                  />
                  <button
                    type="button"
                    className="btn btn-pr3"
                    onClick={recordReset}
                  >
                    신규
                  </button>
          ...
          <div className="card indiv alarm">
            <div className="header d-flex">
              ...
              <button
                ...
                onClick={collapseNoticeArea}
              />
            </div>
            <ul className="scrollbar" role={"noticeList"}>
              {noticeList &&
                noticeList.map((value) => {
                  return <NoticeCard data={value} key={value.noticeSeq} />;
                })}
            </ul>
            <div className="footer">
              <form>
